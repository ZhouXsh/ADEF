# 方法5：Direct Emotion-to-DiT，MEAD 单阶段快速验证版
# 基于 train_Unification_jianhua0803.py：只使用 MEAD，从第 0 步开始训练完整模型。
# 损失保持 baseline 不变，方法性变化只在 emotion conditioning 进入 DiT block 的方式。
...
from src.modules.emotion_dit_Unification_ditcond0819 import DitTalkingHead


g_exp_name = "20260819_Unification_ditcond0819_MEAD200k"
device_id = 2
...
def train(args, model, train_loader, val_loader, optimizer, save_dir,
    ...
    for it in range(start_iter, args.max_iter + 1):
        ...
        # 与 jianhua0803 baseline 完全一致的情感分类损失。
...
if __name__ == '__main__':
    ...
    args.model_variant = 'ditcond0819'
