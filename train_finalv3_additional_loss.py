...
def build_additional_total_loss(args, losses, predict_head_pose: bool):
    """Original train.py losses + train_e2v extra losses.

    Difference from ``build_original_total_loss``:
        - emotion classification loss is controlled by ``l_emo_cls``;
        - emotion level loss is added with ``l_emo_level``;
        - emotion2vec prosody curve loss is added with ``l_prosody_curve``.

    All original motion and head losses are still retained.
    """
    loss = losses["noise"]
    loss = loss + args.l_emo_cls * losses["emo"]
    loss = loss + args.l_emo_level * losses["level"]
    loss = loss + args.l_prosody_curve * losses["prosody"]
...
def append_additional_logs(args, loss_log, losses, predict_head_pose: bool):
    loss_log["noise"].append(losses["noise"].item())
    loss_log["emo"].append((args.l_emo_cls * losses["emo"]).item())
    loss_log["level"].append((args.l_emo_level * losses["level"]).item())
    loss_log["prosody"].append((args.l_prosody_curve * losses["prosody"]).item())
    loss_log["exp"].append((args.l_exp * losses["exp"]).item())
    loss_log["exp_vel"].append((args.l_exp_vel * losses["exp_vel"]).item())
    loss_log["exp_smooth"].append((args.l_exp_smooth * losses["exp_smooth"]).item())
...
def write_additional_scalars(writer, prefix: str, loss_log, current_iter: int):
    write_original_scalars(writer, prefix, loss_log, current_iter)
    writer.add_scalar(f"{prefix}/level_loss", np.mean(loss_log["level"]), current_iter)
    writer.add_scalar(f"{prefix}/prosody_curve_loss", np.mean(loss_log["prosody"]), current_iter)


def describe_additional_losses(loss_log, prefix: str, current_iter=None):
    if current_iter is None:
        msg = f"{prefix} loss: "
    else:
        msg = f"(Iter {current_iter:>6}) {prefix} loss: "
    ...
    msg += (
        f"[N: {np.mean(loss_log['noise']):.3e}, "
        f"Emo: {np.mean(loss_log['emo']):.3e}, "
        f"Level: {np.mean(loss_log['level']):.3e}, "
        f"Prosody: {np.mean(loss_log['prosody']):.3e}, "
        f"EX: {np.mean(loss_log['exp']):.3e}, "
        f"EX_V: {np.mean(loss_log['exp_vel']):.3e}, "
        f"EX_S: {np.mean(loss_log['exp_smooth']):.3e}, "
        f"HA: {np.mean(loss_log['head_angle']):.3e}, "
...
def train(args, model, train_loader, val_loader, optimizer, save_dir, scheduler=None, writer=None, start_iter=0, classifier=None):
    ...
    for it in range(start_iter, args.max_iter + 1):
        ...
        loss = build_additional_total_loss(args, losses, predict_head_pose)
        loss_log["loss"].append(loss.item())
        append_additional_logs(args, loss_log, losses, predict_head_pose)
        ...
        description = describe_additional_losses(loss_log, prefix=f"Iter: {it}\\tTrain")
        logging.info(description)

        if it % args.log_iter == 0 and writer is not None:
            writer.add_scalar("train/total_loss", np.mean(loss_log["loss"]), it)
            write_additional_scalars(writer, "train", loss_log, it)
...
def val(args, model, test_loader, current_iter, n_rounds=1, mode="val", writer=None, classifier=None):
    ...
    description = describe_additional_losses(loss_log, prefix=mode, current_iter=current_iter)
    print(description)

    if writer is not None:
        writer.add_scalar(f"{mode}/total_loss", np.mean(loss_log["loss"]), current_iter)
        write_additional_scalars(writer, mode, loss_log, current_iter)
...
def build_parser():
    ...
    # experiment
    parser.add_argument("--exp_name", type=str, default="20260711_emotion_dit_finalv3_additional_loss")
