...
from src.dataset.dataset_EmotionLevel_2pad_0721 import EmoLevelDataset
from src.modules.emotion_dit_Unification_2pad_0721 import DitTalkingHead
from src.modules.emotion_level_classifier import EmotionTransformer as Classifier
import src.utils as utils


g_exp_name = "20260721_emotion_dit_Unification_2pad_0721"
device_id = 1
...
def _prepare_model_inputs(batch, device):
    ...
    first_motion_pair = [value.to(device) for value in first_motion_pair]
    return (
        audio_pair,
        coef_pair,
        canonical_kp_pair,
...
def _prediction_for_loss(target, prev_motion_coef, args):
    # The two new reference outputs are never supervised or diffused.
...
def _run_pair(args, model, classifier, audio_pair, coef_pair,
              canonical_kp_pair, first_motion_pair, emo_index,
              audio_unit, training):
    ...
    for i in range(2):
        ...
        model_kwargs = dict(
            ...
            first_motion_feat=first_motion_pair[i],
        )
        ...
        if i == 0:
            ...
            if end_idx is not None:
                ...
                prev_motion_coef = returned_motion[
                    :, -args.n_prev_motions:
                ]
                prev_audio_feat = returned_audio[
                    :, -args.n_prev_motions:
                ]
        else:
        ...
        losses = utils.compute_loss_new(
            args, i == 0, motion_coef_in, noise,
            target_for_loss, prev_motion_coef, end_idx,
        )
        (
            loss_n, loss_exp, loss_exp_v, loss_exp_s,
            loss_ha, loss_hv, loss_hs, loss_ht,
        ) = losses

        pred_emo, _ = classifier(current_target[:, :, :63])
        loss_e = cross_criterion(pred_emo, emo_index)

        totals["noise"] = totals["noise"] + loss_n / 2
        totals["emo"] = totals["emo"] + loss_e / 2
        totals["exp"] = _add_half(totals["exp"], loss_exp)
        totals["exp_vel"] = _add_half(totals["exp_vel"], loss_exp_v)
        totals["exp_smooth"] = _add_half(
            totals["exp_smooth"], loss_exp_s
        )
        totals["head_angle"] = _add_half(
            totals["head_angle"], loss_ha
        )
        totals["head_vel"] = _add_half(totals["head_vel"], loss_hv)
        totals["head_smooth"] = _add_half(
            totals["head_smooth"], loss_hs
        )
        if loss_ht is not None:
            totals["head_trans"] = totals["head_trans"] + loss_ht

    loss = totals["noise"] + totals["emo"]
    loss = loss + args.l_exp * totals["exp"]
    loss = loss + args.l_exp_vel * totals["exp_vel"]
    loss = loss + args.l_exp_smooth * totals["exp_smooth"]
    if args.target == "sample" and predict_head_pose:
        loss = loss + args.l_head_angle * totals["head_angle"]
        loss = loss + args.l_head_vel * totals["head_vel"]
        loss = loss + args.l_head_smooth * totals["head_smooth"]
        loss = loss + args.l_head_trans * totals["head_trans"]
    totals["loss"] = loss
    return totals
...
def train(args, model, train_loader, val_loader, optimizer, save_dir,
    ...
    for it in range(start_iter, args.max_iter + 1):
        ...
        audio_pair, coef_pair, canonical_pair, first_pair, emo_index, _ = batch
        totals = _run_pair(
            args, model, classifier, audio_pair, coef_pair,
            canonical_pair, first_pair, emo_index, audio_unit,
            training=True,
        )
        ...
        if it % args.log_iter == 0:
            ...
            _log_values("train", mean_values, writer, it)
        ...
        if it % args.val_iter == 0 or it == 0 or it == args.max_iter:
            val(args, model, val_loader, it, "val", writer, classifier)


@torch.no_grad()
def val(args, model, test_loader, current_iter, mode="val",
        writer=None, classifier=None):
    ...
    for raw_batch in test_loader:
        batch = _prepare_model_inputs(raw_batch, model.device)
        audio_pair, coef_pair, canonical_pair, first_pair, emo_index, _ = batch
        totals = _run_pair(
            args, model, classifier, audio_pair, coef_pair,
            canonical_pair, first_pair, emo_index, audio_unit,
    ...
    mean_values = {key: np.mean(values) for key, values in loss_log.items()}
    logging.info(
        f"(Iter {current_iter:>6}) {mode} loss: "
        f"[N: {mean_values['noise']:.3e}, "
        f"EX: {mean_values['exp']:.3e}, "
        f"Emo: {mean_values['emo']:.3e}, "
        f"Total: {mean_values['loss']:.3e}]"
    )
    _log_values(mode, mean_values, writer, current_iter)
...
def main(args, option_text=None):
    ...
    train(
        ...
        exp_dir / "checkpoints", scheduler, writer,
        start_iter=0, classifier=classifier,
    )
