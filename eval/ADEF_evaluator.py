#!/usr/bin/env python3
"""Evaluate one ADEF experiment directory with paper protocol v4.

Two input formats are supported:
1. ``image,audio,gt_video[,emotion]``: resolve an already-generated ADEF fake
   under ``<father-dir>/<exam_name>`` by the pipeline naming convention.
2. Legacy ``fake_filename,gt_video[,emotion]``.

Evaluation requires only the generated fake video and GT video. Missing fake/GT
samples are written to the failure reports and excluded; all existing samples
continue through the normal paper evaluation pipeline.
"""
...
ADEF_EMOTYPE = {
    "anger": "angry", "contempt": "contempt", "disgust": "disgusted", "fear": "fear",
...
def _detect_row_mode(parts: list[str], path: Path, lineno: int) -> str:
    ...
    if len(parts) == 4:
        if not _is_audio(parts[1]) or not _is_video(parts[2]):
            raise ValueError(f"{path}:{lineno}: four-column input must be image,audio,gt_video,emotion")
        return "triples"
    if len(parts) != 3:
        raise ValueError(f"{path}:{lineno}: expected image,audio,gt_video[,emotion] or fake_filename,gt_video[,emotion]")
    if _is_audio(parts[1]) and _is_video(parts[2]):
        return "triples"
    if _is_video(parts[1]):
        return "pairs"
    raise ValueError(f"{path}:{lineno}: cannot identify input format from column 2: {parts[1]}")


def _resolve_adef_fake(fake_root: Path, image: Path, audio: Path,
                       emotion: str | None) -> tuple[Path, str | None]:
    image_stem, audio_stem = image.stem, audio.stem
    ...
    if emotype:
        ...
        if exact.is_file():
            return exact, None
    candidates = sorted(p for p in fake_root.glob(f"{prefix}*.mp4")
                        if p.is_file() and not p.name.endswith("_temp.mp4"))
    if len(candidates) == 1:
        return candidates[0], None
    if not candidates:
        expected = f"{prefix}{emotype or '<emotion>'}.mp4"
        return fake_root / expected, f"ADEF fake not found; expected pipeline output matching {expected} under {fake_root}"
    return fake_root / f"{prefix}{emotype or '<emotion>'}.mp4", (
        "ambiguous ADEF outputs for image/audio pair: " + ", ".join(p.name for p in candidates)
    )
...
def read_inputs(path: Path, fake_root: Path) -> tuple[list[Sample], list[dict], int, str]:
    ...
    for lineno, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if detected_mode is None:
            ...
            raise ValueError(f"{path}:{lineno}: mixed input formats are not allowed ({detected_mode} then {mode})")

        if mode == "triples":
            image, audio, gt = Path(parts[0]), Path(parts[1]), Path(parts[2])
            ...
            missing = []
            # image/audio are naming metadata at evaluation time; only fake+GT
            # are required for metric computation.
            if not gt.is_file():
                missing.append(f"gt video not found: {gt}")
            if fake_error or not fake.is_file():
                missing.append(fake_error or f"fake video not found: {fake}")
            if missing:
                failures.append({"stage": "input", "name": name, "fake": str(fake), "gt": str(gt),
                                 "error": "; ".join(missing)})
                continue
...
def _clear_outputs(eval_dir: Path) -> None:
    """Clear only presentation/report files; persistent work metric caches stay."""
...
def _write_initial_failures(eval_dir: Path, failures: list[dict]) -> Path:
    path = eval_dir / "failed_samples.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
        fields = ["metric", "name", "fake", "gt", "error"]
        w = csv.DictWriter(f, fieldnames=fields)
        w.writeheader()
        for row in failures:
            w.writerow({"metric": row.get("metric") or row.get("stage") or "input",
                        "name": row.get("name", ""), "fake": row.get("fake", ""),
                        "gt": row.get("gt", ""), "error": row.get("error", "")})
    return path
...
def parse_args():
    ...
    src.add_argument("--triples-file", help="image,audio,gt_video[,emotion]; resolve ADEF fake from experiment directory")
    src.add_argument("--pairs-file", help="Legacy fake,gt[,emotion]. Triples are also auto-detected.")
    ...
    p.add_argument("--force-metrics", nargs="*", default=None,
                   choices=["lse", "fid", "fvd", "pairwise", "emotiefflib", "dfer_clip"],
                   help="Force selected metrics to rerun instead of reusing completed persistent results.")
...
def main() -> int:
    ...
    print(f"[ADEF-eval] input-format={input_mode} requested={expected_n} resolved={len(samples)} missing={len(upstream_failures)}")
    ...
    failure_path = _write_initial_failures(eval_dir, upstream_failures)
    if upstream_failures:
        print(f"[ADEF-eval] {len(upstream_failures)} unavailable sample(s) will be skipped; failures: {failure_path}", file=sys.stderr)
    if not samples:
        print(f"ERROR: all {expected_n} requested samples are unavailable; see {failure_path}", file=sys.stderr)
        return 2

    manifest = write_manifest(eval_dir / "manifest.csv", samples)
    cmd = [sys.executable, str(PAPER_EVALUATOR), "--manifest", str(manifest), "--method", args.exam_name,
           "--output-dir", str(eval_dir), "--device", args.device, "--timeout", str(args.timeout),
           "--expected-n", str(expected_n), "--upstream-failures", str(upstream_path)]
    if args.metrics:
        cmd += ["--metrics", *args.metrics]
    if args.force_metrics is not None:
        cmd += ["--force-metrics", *args.force_metrics]
    ...
    if isinstance(internal_row, dict):
        ...
        print(f"[ADEF-eval] status={report.get('status')} evaluated={report.get('evaluated_n')}/{report.get('expected_n')}")
    ...
    if rc != 0:
        ...
        print("[ADEF-eval] partial evaluation: missing/failed samples were excluded; all available results were retained", file=sys.stderr)
    return rc
