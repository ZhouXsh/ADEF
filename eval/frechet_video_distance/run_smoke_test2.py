"""Smoke test with significantly different real vs fake videos."""
...
def make_synthetic_video(path, num_frames=15, height=224, width=224, color=(0, 0, 0)):
    ...
    frame[:, :] = list(color)  # assign via list to avoid numpy wrap issues
...
def main():
    ...
    for i in range(48):
        # 'real' videos: bright red
        make_synthetic_video(os.path.join(real_dir, "clip_%03d.mp4" % i), color=(200, 30, 30))
        # 'fake' videos: bright green (very different from real)
        make_synthetic_video(os.path.join(fake_dir, "clip_%03d.mp4" % i), color=(30, 200, 30))
