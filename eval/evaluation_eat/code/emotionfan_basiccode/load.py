...
def model_parameters(_structure, _parameterDir):
    ...
    # PyTorch >= 2.6 changed the default of ``weights_only`` to True, which
    # trips on older Emotion-FAN checkpoints because they pickle more than
    # just tensors.  The weights come from the original Open-Debin release
    # so the file is trusted; force weights_only=False to keep loading them.
    try:
        checkpoint = torch.load(_parameterDir, weights_only=False)
    except TypeError:
