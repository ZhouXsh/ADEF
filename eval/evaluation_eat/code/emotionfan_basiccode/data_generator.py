...
import numpy as np
# numpy>=2.0 removed ``numpy.lib.index_tricks``.  The original Emotion-FAN code
# imports ``index_exp`` but never uses it, so a sentinel shim is enough.
try:
    from numpy.lib.index_tricks import index_exp  # noqa: F401
except ImportError:
    class _IndexExp:
...
def load_imgs_tsn(video_root, video_list, rectify_label):
    ...
    with open(video_list, 'r') as imf:
        ...
        for id, line in enumerate(imf):
            ...
            img_lists = glob.glob(video_path+'/*')
    ...
    with open(video_list, 'r') as imf:
        ...
        for id, line in enumerate(imf):
            ...
            for frame in img_lists:
                ...
                imgs_first.append((os.path.join(frame), label))
            ###  return video frame index  #####
