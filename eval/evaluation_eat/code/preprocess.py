...
def process_name(f,name_mode):
    """Parse (pid, emo, lev, vid) from a video filename.

    Modes added/relaxed for MEAD-style flat filenames (see ``evaluate.py``):

    * ``name_mode == 4``  — originally ``{prefix}_{pid}_{emo}_{lev}_{vid}``.
      Now accepts extra trailing tokens (e.g. duplicate embedded GT name + an
      emotion suffix) by reading the LAST FOUR underscore-separated tokens as
      ``pid/emo/lev/vid``.
    * ``name_mode == 7``  — MEAD flat filenames ``{pid}_front_{emo}_level_{lev}_{vid}[_...]``.
      Locates the literal ``"front"`` token, then returns the surrounding
      elements.  This is the mode you want for raw MEAD videos without going
      through the EAT directory layout.
    """
    ...
    if name_mode == 0 :
        ...
        # ATVG gt — strict, must be exactly 4 tokens
        pid , emo , lev , vid  = parts[:4]
    elif name_mode == 3 :
        pid , emo , lev , vid = f.split('-')
    elif name_mode == 4 :
        # Legacy: {prefix}_{pid}_{emo}_{lev}_{vid}
        # Relaxed: accept extra tokens — first is prefix, last 4 are pid/emo/lev/vid.
        if len(parts) >= 5:
        ...
        # Find the literal "front" anchor; if absent, fall back to legacy mode-4 logic.
        try:
            ...
            if len(parts) >= 5:
                ...
                pid, emo, lev, vid = parts[0], 'unk', '1', '000'
    else:
...
def get_parse():
    ...
    args.add_argument('--pid_filter',type=str,default='M003,M030,W009,W015',
                      help='Comma-separated speaker IDs to keep. Empty string disables filtering.')
    return args
...
def evp_process_videos(in_pth, out_crop_pth):
    ...
    for vid_pth in tqdm.tqdm(vlis): 
        ...
        if args.pid_filter and pid not in args.pid_filter.split(','): continue
