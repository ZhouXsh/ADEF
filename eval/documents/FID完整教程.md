...
```
/home/Zhouxishi/VirtualMan_proj/ADEF_remake/eval/pytorch-fid/
├── .venv/                              # 虚拟环境
├── evaluate_fid.py                     # 帧目录版评估脚本
├── evaluate_fid_video.py               # 视频版评估脚本（自动抽帧）
├── EVAL_README.md                      # 简要部署说明
├── USER_GUIDE.md                       # 本教程
├── models_cache/
│   └── pt_inception-2015-12-05-6726825d.pth
└── src/pytorch_fid/                    # pytorch-fid 源码（editable）
```
...
.venv/bin/python evaluate_fid_video.py \
    --path1 /path/to/real_video.mp4 \
    --path2 /path/to/real_video.mp4 \
    --save-stats 2>/dev/null   # 注意：此功能在 evaluate_fid.py
```

> 💡 `evaluate_fid_video.py` 没有 `--save-stats`，需要先抽帧再存，或用 `evaluate_fid.py --save-stats` 处理已抽好的帧目录。
