...
from src.modules.emotion_dit_level import DitTalkingHead
from src.modules.emotion_level_classifier import EmotionTransformer as Classifier
import src.utils as utils


g_exp_name = '20260627_emotion_dit_level_condition'
device_id = int(os.environ.get('ADEF_DEVICE_ID', 0))
...
def _forward_two_clips(args, model, audio_pair, coef_pair, emo_index, emo_level,
                       audio_unit, predict_head_pose, classifier):
    """Shared train/val forward for two consecutive clips.

    Compared with train.py, the fourth dataset output is no longer discarded: emo_level is
    passed into the DiT and is also supervised through the level head of emotion_level_classifier.
    """
    device = model.device
    ...
    emo_index = emo_index.to(device).long()
    emo_level = emo_level.to(device).long()

    if args.use_context_audio_feat:
        audio_feat_context = model.extract_audio_feature(torch.cat(audio_pair, dim=1), args.n_motions * 2)
    else:
        audio_feat_context = None
    ...
    for i in range(2):
        ...
        if (i == 0 and np.random.rand() < args.trunc_prob1) or (i != 0 and np.random.rand() < args.trunc_prob2):
            ...
            if args.use_context_audio_feat:
                audio_in = audio_feat_context[:, i * args.n_motions:(i + 1) * args.n_motions]
            else:
                audio_in = audio
        ...
        if i == 0:
            noise, target, prev_motion_coef, prev_audio_feat = model(
                motion_coef_in, audio_in, indicator=indicator,
                emo_index=emo_index, emo_level=emo_level)
            if end_idx is not None:
                prev_motion_coef = motion_coef[:, -args.n_prev_motions:]
                if args.use_context_audio_feat:
                    prev_audio_feat = audio_feat_context[:, args.n_motions - args.n_prev_motions:args.n_motions].detach()
                else:
                    with torch.no_grad():
                        prev_audio_feat = model.extract_audio_feature(audio)[:, -args.n_prev_motions:].detach()
            else:
            ...
            noise, target, _, _ = model(
                ...
                indicator=indicator, emo_index=emo_index, emo_level=emo_level)
...
def _compose_loss(args, losses, predict_head_pose):
    loss = losses['noise']
    loss = loss + losses['emo']
    loss = loss + args.l_level * losses['level']
...
def _log_losses(args, loss_log, losses, loss, predict_head_pose):
    loss_log['loss'].append(loss.item())
    loss_log['noise'].append(losses['noise'].item())
    loss_log['emo'].append(losses['emo'].item())
    loss_log['level'].append(losses['level'].item() * args.l_level)
...
def _description(prefix, loss_log, args, predict_head_pose):
    desc = f'{prefix} loss: [N: {np.mean(loss_log["noise"]):.3e}'
    desc += f", EX: {np.mean(loss_log['exp']):.3e}"
    desc += f", EX_V: {np.mean(loss_log['exp_vel']):.3e}"
    desc += f", EX_S: {np.mean(loss_log['exp_smooth']):.3e}"
    if args.target == 'sample' and predict_head_pose and args.l_head_angle > 0:
        desc += f", HA: {np.mean(loss_log['head_angle']):.3e}"
    if args.target == 'sample' and predict_head_pose and args.l_head_vel > 0:
        desc += f", HV: {np.mean(loss_log['head_vel']):.3e}"
    if args.target == 'sample' and predict_head_pose and args.l_head_smooth > 0:
        desc += f", HS: {np.mean(loss_log['head_smooth']):.3e}"
    if args.target == 'sample' and predict_head_pose and args.l_head_trans > 0:
        desc += f", HT: {np.mean(loss_log['head_trans']):.3e}"
    desc += f", Emo: {np.mean(loss_log['emo']):.3e}"
    desc += f", Level: {np.mean(loss_log['level']):.3e}"
    desc += ']'
    return desc


def train(args, model, train_loader, val_loader, optimizer, save_dir, scheduler=None,
          writer=None, start_iter=0, classifier=None):
    ...
    for it in range(start_iter, args.max_iter + 1):
        audio_pair, coef_pair, emo_index, emo_level = next(data_loader)
        losses = _forward_two_clips(args, model, audio_pair, coef_pair, emo_index, emo_level,
                                    audio_unit, predict_head_pose, classifier)
        loss = _compose_loss(args, losses, predict_head_pose)
        loss.backward()
        ...
        _log_losses(args, loss_log, losses, loss, predict_head_pose)
        logging.info(_description(f'Iter: {it}\t Train', loss_log, args, predict_head_pose))
...
def val(args, model, test_loader, current_iter, n_rounds=1, mode='val', writer=None, classifier=None):
    ...
    for _ in range(n_rounds):
        for audio_pair, coef_pair, emo_index, emo_level in test_loader:
            losses = _forward_two_clips(args, model, audio_pair, coef_pair, emo_index, emo_level,
                                        audio_unit, predict_head_pose, classifier)
            loss = _compose_loss(args, losses, predict_head_pose)
            _log_losses(args, loss_log, losses, loss, predict_head_pose)

    desc = _description(f'(Iter {current_iter:>6}) {mode}', loss_log, args, predict_head_pose)
    print(desc)
...
def main(args, option_text=None):
    model_kwargs = dict(
        ...
        emo_levels=args.emo_levels,
        default_emo_level=args.default_emo_level,
    )
    ...
    classifier.load_state_dict(torch.load('pretrained_weights/ADEF/emo_classifier/emo_level_classifier.pth',
    ...
    train(args, model, train_loader, val_loader, optimizer, exp_dir / 'checkpoints',
          scheduler, writer, start_iter=start_iter, classifier=classifier)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--emo_levels', type=int, default=3, help='number of emotion intensity levels')
    parser.add_argument('--default_emo_level', type=int, default=1, help='fallback level: 0/1/2 means level 1/2/3')
    ...
    parser.add_argument('--l_level', type=float, default=1.0, help='weight of the emotion level classification loss')
    ...
    option_text = utils.common.get_option_text(args, parser) if args.mode == 'train' else None
