...
def crop_image(img, pts: np.ndarray, **kwargs):
    ...
    ret_dct = {
        'M_o2c': M_o2c,  # from the original image to the cropped image 3x3   从原始图像到裁剪图像3x3
        'M_c2o': M_c2o,  # from the cropped image to the original image 3x3   从裁剪图像到原始图像3x3
        'img_crop': img_crop,  # the cropped image                            裁剪图像
        'pt_crop': pt_crop,  # the landmarks of the cropped image             裁剪图像的landmarks
    }
