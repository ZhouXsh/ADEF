...
class Cropper(object):
    ...
    # 已裁剪的图像计算landmark
    def calc_lmk_from_cropped_image(self, img_rgb_, **kwargs):   # img_rgb_：(h,w,3)
        ...
        # src_face:检测结果，包含 'bbox', 'kps', 'det_score', 'landmark_2d_106'
        if len(src_face) == 0:  # 没脸
            log("No face detected in the source image.")
            return None
        elif len(src_face) > 1:  # 多于1张脸
            log(f"More than one face detected in the image, only pick one face by rule {direction}.")
        src_face = src_face[0]
        lmk = src_face.landmark_2d_106
        lmk = self.human_landmark_runner.run(img_rgb_, lmk)   # (203, 2)
    ...
    def crop_driving_video(self, driving_rgb_lst, **kwargs):
        """Tracking based landmarks/alignment and cropping 基于跟踪的地标/对齐和裁剪。
        检测人脸、提取人脸关键点、计算人脸运动轨迹，并进行人脸对齐和裁剪
        """
        trajectory = Trajectory()  # 用于存储人脸跟踪数据，包括关键点 (lmk_lst)、边界框 (bbox_lst)、裁剪后帧 (frame_rgb_crop_lst) 等
        direction = kwargs.get("direction", "large-small")   # 用于控制如何选择人脸，比如从大到小的方式筛选人脸。
        for idx, frame_rgb in enumerate(driving_rgb_lst):
            if idx == 0 or trajectory.start == -1:   # 处理第一帧 或 第一个人脸帧
                src_face = self.face_analysis_wrapper.get(   # 提取 106 个 2D 关键点
                    contiguous(frame_rgb[..., ::-1]),        # 转换 RGB 到 BGR
                    flag_do_landmark_2d_106=True,
                    direction=direction,
                )
                if len(src_face) == 0:   # 没有检测到人脸
                    log(f"No face detected in the frame #{idx}")
                    continue
                elif len(src_face) > 1:  # 检测到多张人脸：按照 direction 规则选择其中一个（默认 "large-small" 选择较大的人脸）
                    log(f"More than one face detected in the driving frame_{idx}, only pick one face by rule {direction}.")
                src_face = src_face[0]
                lmk = src_face.landmark_2d_106               # 提取 106 个 2D 关键点
                lmk = self.human_landmark_runner.run(frame_rgb, lmk)  # 进一步优化关键点
                trajectory.start, trajectory.end = idx, idx  # 记录人脸检测的起始帧索引 (start) 和结束帧索引 (end)
            else:
                lmk = self.human_landmark_runner.run(frame_rgb, trajectory.lmk_lst[-1])  # 从上一个结束帧开始
                trajectory.end = idx

            trajectory.lmk_lst.append(lmk)   # 更新人脸运动轨迹
            ret_bbox = parse_bbox_from_landmark(   # 计算人脸裁剪边界框
                lmk,
        ...
        # 计算全局人脸边界框（计算所有帧的人脸 bbox 均值，用于确保裁剪区域一致，避免画面抖动）
        global_bbox = average_bbox_lst(trajectory.bbox_lst)
        # 执行裁剪
        for idx, (frame_rgb, lmk) in enumerate(zip(trajectory.frame_rgb_lst, trajectory.lmk_lst)):
            ret_dct = crop_image_by_bbox(        # 根据 global_bbox 裁剪视频帧，并返回裁剪后的人脸关键点
                frame_rgb,
                global_bbox,
                lmk=lmk,
                dsize=kwargs.get("dsize", 512),  # 输出裁剪图大小（默认 512x512）。
                flag_rot=False,
                borderValue=(0, 0, 0),           # 填充边缘为黑色。
            )
            trajectory.frame_rgb_crop_lst.append(ret_dct["img_crop"])
            trajectory.lmk_crop_lst.append(ret_dct["lmk_crop"])

        return {
            "frame_crop_lst": trajectory.frame_rgb_crop_lst,   # 裁剪后的视频帧
            "lmk_crop_lst": trajectory.lmk_crop_lst,           # 裁剪后的人脸关键点
        }


    def calc_lmks_from_cropped_video(self, driving_rgb_crop_lst, **kwargs):
        """Tracking based landmarks/alignment 基于跟踪的地标/对齐"""
        trajectory = Trajectory()
        direction = kwargs.get("direction", "large-small")
        ...
        for idx, frame_rgb_crop in enumerate(driving_rgb_crop_lst):
            ...
            trajectory.lmk_lst.append(lmk)
        return trajectory.lmk_lst          # 裁剪后的人脸关键点
