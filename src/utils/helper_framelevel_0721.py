...
"""
Independent utility helpers for the framelevel (emotion2vec frame-level) pipeline.

This file is a verbatim copy of `src/utils/helper.py` with one change:
the `motion_generator` branch in `load_model` builds
`emotion_dit_Unification_framelevel_0721.DitTalkingHead` (and explicitly
forwards `emotion2vec_dim`). Everything else (utility functions, F / M /
W / G / S-R loading) is intentionally identical so the framelevel
pipeline does not depend on the original `helper.py` at all.
"""
import os
...
def load_model(ckpt_path, model_config, device, model_type):
    ...
    if model_type == 'appearance_feature_extractor':         # 外观特征提取器 F
    ...
    elif model_type == 'motion_generator':                    # 运动生成器 (frame-level)
        model_data = torch.load(ckpt_path, map_location=device)
        model_args = NullableArgs(model_data['args'])
        emotion2vec_dim = _get_arg(model_args, "emotion2vec_dim", 1024)
        ...
        # 过滤掉 shape 不匹配的参数（PositionalEncoding.pe 之类的 buffer 由当前模型按 n_diff_steps 重建）
