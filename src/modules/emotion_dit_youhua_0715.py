...
在/home/Zhouxishi/VirtualMan_proj/ADEF_remake/src/modules/emotion_dit_youhua_0714.py的基础上进一步优化，包括：
当无情感条件时，不再使用emotion_feat_null进行调制，而是直接不调制了！！！！
self.null_emotion_feat = nn.Parameter(torch.zeros(1, 1, emotion_feat_dim))改成self.null_emotion_feat = torch.zeros(1, 1, emotion_feat_dim)，从而完全替换为0向量，而不是学习的参数。
forward中的prev_audio_feat_，改成与audio_feat_一样的调制位置。而不是提前调制了。
'''
...
class DitTalkingHead(nn.Module):
    def __init__(self, device='cuda', target="sample", architecture="decoder",  
        ...
        if 'emotion' in self.guiding_conditions:   # True
            ...
            # self.null_emotion_feat = torch.zeros(1, 1, emotion_feat_dim) # 1, 1, 512
            self.register_buffer('null_emotion_feat', torch.zeros(1, 1, emotion_feat_dim))
        ...
        if len(self.guiding_conditions) > 0:         # ['audio']
            ...
            if len(self.guiding_conditions) == 1 or self.cfg_mode == 'independent':   # True只有音频作为条件      self.cfg_mode == "incremental"
                ...
                if 'audio' in self.guiding_conditions:   # True
                    ...
                    prev_audio_feat_ = torch.where(mask_audio.view(-1, 1, 1),        # null_audio_feat：随机值组成的张量，代表没有特征
                                             ...
                                             prev_audio_feat)      # (N=8, L=100, feature_dim=512)
            else:   # 不必理会
            ...
            if len(self.guiding_conditions) == 2 and 'emotion' in self.guiding_conditions:
                ...
                emo_feat = torch.where(mask_emotion.view(-1, 1, 1),       #  (b,1,1)
                                        ...
                                        emo_feat)                # emo_feat: (N, 1, 512)
                emo_shift, emo_scale = self.adaLN_modulation(emo_feat).chunk(2, dim=2)  # (N, 1, 512),  (N, 1, 512)
                audio_feat = self.audio_norm(audio_feat) * (1 + emo_scale) + emo_shift
                prev_audio_feat_ = self.audio_norm(prev_audio_feat_) * (1 + emo_scale) + emo_shift
    ...
    def sample(self, audio_or_feat, prev_motion_feat=None, prev_audio_feat=None,
        ...
        if prev_motion_feat is None:  # (N, n_prev_motions=10, motion_feat_dim=70)
            ...
            prev_motion_feat = torch.index_select(self.start_motion_feat, 0, emo_index) # （1, n_prev_motions=10, motion_feat_dim=70） -> (N, n_prev_motions=10, motion_feat_dim=70)
        pre_None = False
        ...
        if 'audio' in cfg_cond:        # 音频作为条件     this
            audio_feat_null = self.null_audio_feat.expand(batch_size, self.n_motions, -1)  # (1, 1, feature_dim=512) -> (N=1, L=100, feature_dim=512) 随机值
        else: 
            audio_feat_null = audio_feat   # (N, L=100, feature_dim=512)  L:帧数

        if 'emotion' in cfg_cond:
            audio_feat_null = self.audio_norm(audio_feat_null)
            audio_no_emotion = self.audio_norm(audio_feat)
            prev_audio_feat_null= self.audio_norm(self.null_audio_feat.expand(batch_size, self.n_prev_motions, -1))
            prev_audio_feat_no_emotion= self.audio_norm(prev_audio_feat)
        ...
        for t in range(self.diffusion_sched.num_steps, 0, -1):   # 50~1   -1表示逆序遍历
            ...
            results = self.denoising_net(motion_in, audio_feat_in, prev_motion_feat_in,      # result = (N*2=2, L_p + L= 10+100 = 110, motion_feat_dim=70)
...
class DenoisingNetwork(nn.Module):
    def __init__(self, device='cuda', motion_feat_dim=70, 
        ...
        super().__init__()
        # Model parameters
        self.motion_feat_dim = motion_feat_dim     # 推理70
