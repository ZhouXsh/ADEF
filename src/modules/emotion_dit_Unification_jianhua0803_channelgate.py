...
class DitTalkingHead(nn.Module):
    ...
    def apply_emotion_modulation(self, audio_feat, shift, scale):
    ...
    def forward(self, motion_feat, audio_or_feat, prev_motion_feat=None,
        ...
        audio_feat_cond = self.apply_emotion_modulation(audio_feat, emo_shift, emo_scale)

        if prev_audio_is_start:
            prev_audio_feat = self.audio_norm(prev_audio_feat)
        else:
            prev_audio_feat = (
                self.apply_emotion_modulation(prev_audio_feat, emo_shift, emo_scale)
            )
        ...
        audio_feat_uncond = (
            self.apply_emotion_modulation(null_audio_feat, null_shift, null_scale)
        )
    ...
    def _forward_independent_cfg(self, motion_feat, audio_or_feat,
        ...
        if 'emotion' in self.guiding_conditions:
            ...
            if pre_None:
                ...
                prev_audio_feat = self.apply_emotion_modulation(prev_audio_feat, emo_shift, emo_scale)
        ...
        if len(self.guiding_conditions) > 0:
            ...
            if len(self.guiding_conditions) == 2 and 'emotion' in self.guiding_conditions:
                ...
                emo_shift, emo_scale = self.adaLN_modulation(emo_feat).chunk(2, dim=2)
                audio_feat = self.apply_emotion_modulation(audio_feat, emo_shift, emo_scale)
    ...
    def sample(self, audio_or_feat, prev_motion_feat=None, prev_audio_feat=None,
        ...
        emo_shift, emo_scale = self.adaLN_modulation(emo_feat).chunk(2, dim=2)
        audio_feat_cond = (
            self.apply_emotion_modulation(audio_feat_saved, emo_shift, emo_scale)
        )
