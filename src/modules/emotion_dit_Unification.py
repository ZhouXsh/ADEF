## 大一统版本。基于/home/Zhouxishi/VirtualMan_proj/ADEF_remake/src/modules/emotion_dit_SingleCFG_0717.py
## 时间步调制：改为复用 emotion_dit_timestep_0714 的去噪网络。该版本的 DenoisingNetwork 使用 adaLN-Zero 风格的 DiTDecoder，将扩散时间步以 FiLM(shift/scale/gate) 的形式逐层注入到每个 block 的自注意力/交叉注意力/前馈之间，而不是在入口一次性相加，
...
class DitTalkingHead(BaseDitTalkingHead):
    ...
    def forward(self, motion_feat, audio_or_feat, prev_motion_feat=None,
                ...
                emo_index=None):
        # Preserve the original behavior for non-joint configurations.
        joint_cfg = (
