...
"""
Independent ADEF pipeline for the framelevel (emotion2vec frame-level)
inference.

This is a verbatim copy of `src/ADEF_pipeline.py` with two changes:
  1. `ADEFWrapper` import is redirected to `ADEF_wrapper_framelevel_0721`.
  2. The motion-template cache path is hard-coded to
     `<audio>_framelevel_0721.pkl`, so it never reuses a `.pkl` written
     by the original (no-CFG) pipeline.

No inheritance from `ADEFPipeline`, so the framelevel pipeline does not
depend on the original `ADEF_pipeline.py` at all.
"""
...
from .utils.helper_framelevel_0721 import (
    mkdir,
    basename,
    dct2device,
    is_image,
...
from .ADEF_wrapper_framelevel_0721 import ADEFWrapper
...
def _framelevel_template_path(audio_path):
    """framelevel 专属的 motion 缓存路径，避免与原版 pipeline 共享 .pkl。"""
    return remove_suffix(audio_path) + '_framelevel_0721.pkl'
...
######## load reference image  加载参考图像########
        ...
        #  从模版中读取motion  （用于验证，加速驱动）
            # framelevel 用独立缓存：<audio>_framelevel_0721.pkl
            wfp_template = _framelevel_template_path(args.audio)
