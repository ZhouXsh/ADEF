...
# l_exp  0.1 to 1.0
# mask： 2
from src.modules.emotion_dit_Unification_jianhua0803_wide_minsnr import DitTalkingHead
g_exp_name = "20260823_matrix_A2B1_wide640_L8_minsnr_adamw"
device_id = 4

# 640 维、8 层、10 头 AdamW + Min-SNR
...
def stratified_diffusion_timesteps(batch_size, num_steps, device):
    """Unbiased, low-variance coverage of the full diffusion horizon per batch."""
...
def min_snr_diffusion_loss(args, model, is_starting_sample, motion_coef_gt,
                           noise, target, prev_motion_coef, end_idx, time_step):
    """Min-SNR-gamma weighting for the primary diffusion objective only."""
    if args.target == 'noise':
        prediction = target[:, args.n_prev_motions:]
        ground_truth = noise
        mask = torch.ones(
        ...
        if is_starting_sample:
            prediction = target[:, args.n_prev_motions:]
            ground_truth = motion_coef_gt
            mask = torch.ones(
                prediction.shape[:2], dtype=torch.bool, device=prediction.device
            )
        else:
            prediction = target
            ground_truth = torch.cat([prev_motion_coef, motion_coef_gt], dim=1)
            current_mask = torch.ones(
                (target.shape[0], args.n_motions),
                dtype=torch.bool,
                device=target.device,
            )
            mask = torch.cat([
                torch.zeros(
                    (target.shape[0], args.n_prev_motions),
                    dtype=torch.bool,
                    device=target.device,
    ...
    if end_idx is not None:
        current_mask = torch.arange(args.n_motions, device=target.device).expand(
            target.shape[0], -1
        ) < end_idx.unsqueeze(1)
    ...
    if args.criterion.lower() == 'l2':
        elementwise = (prediction - ground_truth).square()
    elif args.criterion.lower() == 'l1':
        elementwise = (prediction - ground_truth).abs()
    else:
    ...
    if args.target == 'sample':
        weights = torch.minimum(snr, torch.full_like(snr, args.min_snr_gamma))
    else:
        weights = torch.minimum(snr, torch.full_like(snr, args.min_snr_gamma)) / snr.clamp_min(1e-8)
...
def main(args, option_text=None):
    ...
    for name, parameter in model.named_parameters():
        if not parameter.requires_grad:
            continue
        if parameter.ndim < 2 or any(token in name for token in ('norm', 'embed', 'start_', 'null_')):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--feature_dim', type=int, default=640, help='dimension of the hidden feature')
    ...
    parser.add_argument('--cos_max_iter', type=int, default=300000)
